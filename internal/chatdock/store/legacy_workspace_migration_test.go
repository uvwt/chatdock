...
func TestMigrateLegacyWorkspacesPreservesDataAndBuildsProjects(t *testing.T) {
	...
	providers, err := store.ListModelProviders()
	if err != nil {
		t.Fatal(err)
	}
	...
	assertMigratedSession(t, store.db, "s-default", "", "default", "global-model")
	assertMigratedSession(t, store.db, "s-inherit", "", "", "")
	assertMigratedSession(t, store.db, "s-mcp", "MCP", "legacy-mcp", "mcp-model")
	for table, want := range map[string]int64{
		...
		assertTableCount(t, store.db, table, want)
	}
	for _, table := range []string{"workspaces", "workspace_kv", "schema_migrations"} {
		if exists, err := sqliteTableExists(store.db, table); err != nil {
			t.Fatal(err)
		} else if exists {
...
func TestMigrateLegacyWorkspacesRejectsUnsafeInputs(t *testing.T) {
	...
	t.Run("current schema", func(t *testing.T) {
		sourceDir := t.TempDir()
		current, err := NewStore(sourceDir)
		if err != nil {
			t.Fatal(err)
		}
...
func createLegacyWorkspaceMigrationFixture(t *testing.T) string {
	...
	providers := []modelProviderRecord{
		normalizeModelProviderRecord(modelProviderRecord{ID: "default", Name: "Global", Type: "openai-compatible", BaseURL: globalConfig.BaseURL, APIKeys: upsertProviderAPIKey(nil, "", globalConfig.APIKey, time.Now()), DefaultModel: globalConfig.Model, Models: globalConfig.Models, TimeoutMS: 120000, Enabled: true, CreatedAt: time.Now(), UpdatedAt: time.Now()}),
		normalizeModelProviderRecord(modelProviderRecord{ID: "mcp", Name: "Existing MCP", Type: "openai-compatible", BaseURL: "https://other-mcp.example/v1", APIKeys: upsertProviderAPIKey(nil, "", "other-key", time.Now()), DefaultModel: "other-model", Models: []string{"other-model"}, TimeoutMS: 120000, Enabled: enabled, CreatedAt: time.Now(), UpdatedAt: time.Now()}),
	}
	if err := saveModelProviderRecordsWith(db, providers); err != nil {
		t.Fatal(err)
	}
...
func hasProvider(providers []ModelProvider, id string, baseURL string, modelName string) bool {
