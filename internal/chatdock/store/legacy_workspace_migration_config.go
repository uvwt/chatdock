...
func resolveLegacyWorkspaceProviders(reader sqlQueryer, plan legacyWorkspaceMigrationPlan, records []modelProviderRecord) (map[string]LegacyWorkspaceMigrationProvider, []modelProviderRecord, []LegacyWorkspaceMigrationProvider, error) {
...
func ensureLegacyWorkspaceProvider(records []modelProviderRecord, workspace legacyWorkspaceRecord, cfg model.ModelConfig) (string, []modelProviderRecord, bool, error) {
	desiredID := normalizeProviderID(cfg.ProviderID)
	if desiredID == "" {
		desiredID = "legacy-" + normalizeProviderID(workspace.Name)
	}
	for i := range records {
		if records[i].ID != desiredID {
			continue
		}
		selectedKey, err := selectedAPIKeyForRecord(records[i])
		if err != nil {
		...
		if sameEndpoint && sameCredential {
			records[i].Models = normalizeProviderModelNames(records[i].Models, cfg.Model)
	...
	baseID := "legacy-" + normalizeProviderID(workspace.Name)
	if baseID == "legacy-" {
		baseID = "legacy-workspace"
	}
	providerID := uniqueProviderID(baseID, records)
	...
	for _, workspace := range workspaces {
		...
		for _, name := range names {
			...
			if existing, exists := merged.Servers[candidate]; exists {
				...
				base := name + "-" + normalizeProviderID(workspace.Name)
				candidate = base
				for suffix := 2; ; suffix++ {
...
func copyNonLegacyMeta(source sqlQueryer, target sqlWriter) error {
	...
	for rows.Next() {
		...
		if key == modelProvidersMetaKey || legacyMigrationMetaKeys[key] {
			continue
		}
