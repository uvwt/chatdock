...
func copyLegacySessions(source sqlQueryer, target sqlWriter, plan legacyWorkspaceMigrationPlan, workspaceProviders map[string]LegacyWorkspaceMigrationProvider, providers []modelProviderRecord) (legacySessionCopyReport, error) {
	...
	providerByID := make(map[string]modelProviderRecord, len(providers))
	for _, provider := range providers {
		providerByID[provider.ID] = provider
	}
	globalProvider := providerByID[normalizeProviderID(plan.GlobalConfig.ProviderID)]
...
func providerSupportsModel(provider modelProviderRecord, modelName string) bool {
	if provider.ID == "" {
		return false
	}
	modelName = strings.TrimSpace(modelName)
...
func validateMigratedWorkspaceDatabase(db *sql.DB, plan legacyWorkspaceMigrationPlan, report LegacyWorkspaceMigrationReport) error {
	...
	if _, err := loadModelProviderRecordsWith(db); err != nil {
