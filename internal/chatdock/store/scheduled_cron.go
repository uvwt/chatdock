...
func normalizeCronSchedule(expressions []string, timezone string, now time.Time) ([]string, string, time.Time, error) {
	timezone, location, err := normalizeScheduleTimezone(timezone)
	if err != nil {
...
func nextCronRun(now time.Time, expressions []string, timezone string) (time.Time, error) {
	_, location, err := normalizeScheduleTimezone(timezone)
	if err != nil {
...
func normalizeScheduleTimezone(value string) (string, *time.Location, error) {
	value = strings.TrimSpace(value)
	if value == "" {
		value = configuredScheduleTimezone()
	}
...
func scheduleLocation() *time.Location {
	location, _ := time.LoadLocation(configuredScheduleTimezone())
	return location
}
