...
func (s *Store) ListAgentTasks(limit int) (AgentTaskResponse, error) {
	...
	s.mu.RLock()
	defer s.mu.RUnlock()
	rows, err := s.db.Query(`SELECT r.session_id, e.run_id, e.server, e.tool, e.action, e.status, e.summary, e.arguments_json, e.result_json, e.error, e.created_at
FROM mcp_run_events e JOIN mcp_runs r ON r.id = e.run_id
WHERE e.tool = 'task_manage'
ORDER BY e.created_at DESC LIMIT ?`, limit*4)
	if err != nil {
	...
	for rows.Next() {
		var sessionID, runID, server, tool, action, status, summary, argsRaw, resultRaw, errorText, createdRaw string
		if err := rows.Scan(&sessionID, &runID, &server, &tool, &action, &status, &summary, &argsRaw, &resultRaw, &errorText, &createdRaw); err != nil {
			return AgentTaskResponse{}, err
		}
		_ = tool
