...
func TestScheduledTaskRunListIncludesGeneratedSessionTitle(t *testing.T) {
	...
	if _, err := store.FinishScheduledTaskRun(defaultWorkspaceID, run.Task.ID, run.RunID, run.SessionID, "今天适合专注开发。", startedAt, true, nil, false); err != nil {
		t.Fatal(err)
	}
	if _, err := store.RenameSession(defaultWorkspaceID, run.SessionID, "今日晨间摘要"); err != nil {
		t.Fatal(err)
	}

	runs, err := store.ListScheduledTaskRuns(defaultWorkspaceID, run.Task.ID, 10)
	if err != nil {
		t.Fatal(err)
	}
