...
func TestBuiltinModelProviderToolCRUDAndGlobalDefault(t *testing.T) {
	...
	global := enabledResult["global"].(map[string]any)
	if global["provider_id"] != provider.ID || global["model"] != "other-model" {
