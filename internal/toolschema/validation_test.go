...
func TestValidateToolArgumentsChecksRequiredNestedAndAdditionalSchemas(t *testing.T) {
	...
	for name, tc := range cases {
		t.Run(name, func(t *testing.T) {
			if err := ValidateArguments(schema, tc.args); err == nil || !strings.Contains(err.Error(), tc.want) {
...
func TestValidateToolArgumentsRejectsDisallowedUnknownFields(t *testing.T) {
	...
	if err := ValidateArguments(schema, map[string]any{"known": true}); err != nil {
		t.Fatal(err)
	}
	if err := ValidateArguments(schema, map[string]any{"unknown": true}); err == nil || !strings.Contains(err.Error(), "arguments.unknown is not allowed") {
