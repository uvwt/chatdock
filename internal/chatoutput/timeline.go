...
func (r *timelineRecorder) Record(event string, value any) {
	switch event {
	case "delta":
		r.recordDelta(value)
...
func (r *timelineRecorder) recordDelta(value any) {
...
func (r *timelineRecorder) appendTextPart(kind string, text string) {
	if text == "" {
		return
	}
	last := len(r.parts) - 1
...
func (r *timelineRecorder) recordToolStart(value any) {
	data := mapValue(value)
...
func (r *timelineRecorder) recordToolResult(value any) {
	data := mapValue(value)
	tool := stringValue(data["tool"])
	if tool == "" {
		return
...
func (r *timelineRecorder) Parts() []model.MessagePart {
	return r.parts
}

func (r *timelineRecorder) Events() []model.MessageEvent {
	return r.events
}

func (r *timelineRecorder) Empty() bool {
	return len(r.parts) == 0 && len(r.events) == 0
}
