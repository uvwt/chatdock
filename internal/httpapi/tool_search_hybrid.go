...
func (a *Server) searchToolCatalog(ctx context.Context, catalog toolCatalog, args map[string]any) map[string]any {
	result, _ := searchToolCatalogWithMatches(ctx, a, catalog, args)
	return result
}

func searchToolCatalogWithApp(ctx context.Context, app *Server, catalog toolCatalog, args map[string]any) map[string]any {
	result, _ := searchToolCatalogWithMatches(ctx, app, catalog, args)
	return result
}

func searchToolCatalogWithMatches(ctx context.Context, app *Server, catalog toolCatalog, args map[string]any) (map[string]any, []mcp.MCPTool) {
...
func (a *Server) semanticToolScores(ctx context.Context, catalog toolCatalog, query string) map[string]float64 {
...
func (a *Server) cachedQueryEmbedding(ctx context.Context, cfg model.ModelConfig, query string) ([]float64, bool) {
...
func (a *Server) clearQueryEmbeddingCache() {
	a.embeddingMu.Lock()
	a.embeddingMemo = make(map[string][]float64)
	a.embeddingMu.Unlock()
}

func (a *Server) ensureToolEmbeddingIndex(ctx context.Context, catalog toolCatalog, model string) (map[string]storepkg.ToolEmbeddingRecord, error) {
...
func (a *Server) availableToolEmbeddingIndex(catalog toolCatalog, model string) (map[string]storepkg.ToolEmbeddingRecord, error) {
	model = strings.TrimSpace(model)
	if model == "" {
		model = "BAAI/bge-m3"
	}
...
func (a *Server) saveMissingToolEmbeddings(ctx context.Context, model string, existing map[string]storepkg.ToolEmbeddingRecord, catalog toolCatalog) (map[string]storepkg.ToolEmbeddingRecord, error) {
	cfg := a.embeddingConfig()
...
func (a *Server) toolSearchMode() string {
...
func (a *Server) embeddingConfig() model.ModelConfig {
