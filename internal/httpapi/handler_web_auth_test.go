...
func TestSPAFallbackAndBackendBoundary(t *testing.T) {
	...
	app, err := NewServer(model.ServerConfig{Addr: "127.0.0.1:0", DataDir: t.TempDir(), WebDir: webDir})
	if err != nil {
		t.Fatal(err)
	}
	routes := app.routes()
...
func TestSecurityHeadersApplyToWebAssetsAndUnauthorizedAPI(t *testing.T) {
	...
	app, err := NewServer(model.ServerConfig{Addr: "127.0.0.1:0", DataDir: t.TempDir(), WebDir: webDir, AuthToken: "secret"})
	if err != nil {
		t.Fatal(err)
	}
	t.Cleanup(func() {
...
func TestAuthLoginWithCredential(t *testing.T) {
	...
	app, err := NewServer(model.ServerConfig{Addr: "127.0.0.1:0", DataDir: t.TempDir(), WebDir: webDir, AuthToken: "server-token", AuthUsername: "admin", AuthCredential: "demo-value"})
	if err != nil {
		t.Fatal(err)
	}
	routes := app.routes()
...
func TestAuthLoginRequiresConfiguredCredential(t *testing.T) {
	...
	app, err := NewServer(model.ServerConfig{Addr: "127.0.0.1:0", DataDir: t.TempDir(), WebDir: webDir, AuthToken: "server-token"})
	if err != nil {
		t.Fatal(err)
	}
	routes := app.routes()
