...
const failures = [];
const budgets = {
  exactDuplicateBlocks: 50,
  selectorsRepeatedAtLeast4: 110,
  maxSelectorLayers: 27,
};
...
for (const file of listCSS(stylesDir)) {
  ...
  for (const match of text.matchAll(/([^{}]+)\{([^{}]*)\}/g)) {
    ...
    const blockKey = selector + '\n' + body;
    exactBlocks.set(blockKey, [...(exactBlocks.get(blockKey) || []), rel]);
    selectorFiles.set(selector, [...(selectorFiles.get(selector) || []), rel]);
  }
}
...
console.log(`css health ok: exact_duplicate_blocks=${duplicateBlockCount}/${budgets.exactDuplicateBlocks}, repeated_selectors>=4=${repeatedSelectorCount}/${budgets.selectorsRepeatedAtLeast4}`);
