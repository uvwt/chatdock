...
export function SettingsPanel(props) {
  const {
    ...
    loadSystemStatus, logout, mcpConfig, mcpStatus, onCopy, providers, promptPreview, refreshProductState, refreshVisibleSettings, runScheduledTaskNow, viewScheduledTaskRuns, runSetupWizard,
...
function TaskCard({ task, editScheduledTask, deleteScheduledTask, toggleScheduledTask, runScheduledTaskNow, viewScheduledTaskRuns }) {
  return <div className="task-card"><div className="task-head"><div><div className="task-name">{task.title || '未命名任务'}{task.running ? ' · 运行中' : ''}</div><div className="task-desc">{(task.prompt || '').slice(0, 120) || '无提示内容'}</div></div><div className="task-actions"><span className={'badge ' + taskStatusClass(task)}>{taskStatusLabel(task)}</span><button className="secondary small" disabled={task.running} onClick={() => runScheduledTaskNow(task.id)}>立即运行</button><button className="secondary small" onClick={() => viewScheduledTaskRuns(task.id)}>查看记录</button><button className="secondary small" disabled={task.running} onClick={() => editScheduledTask(task.id)}>编辑</button><button className="danger small" disabled={task.running} onClick={() => deleteScheduledTask(task.id)}>删除</button></div></div><label className="task-toggle"><input type="checkbox" disabled={task.running} checked={!!task.enabled} onChange={e => toggleScheduledTask(task.id, e.target.checked)} /> 启用</label><div className="task-meta">{scheduleSummary(task)}</div><div className="task-meta">上下文：{scheduledTaskContextLabel(task.context_mode || 'stateless')}{task.context_mode === 'session' && task.session_id ? ' · 会话 ' + task.session_id : ''}</div>{task.running ? <div className="hint">任务运行中，暂时不能编辑、删除或切换启用状态。</div> : null}{task.last_error ? <div className="task-error">上次错误：{task.last_error}</div> : null}</div>;
}
