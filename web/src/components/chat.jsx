...
export function EmptyState({ openSettings, openWorkspacePicker, busy, hasWorkspaces, setInput, modelReady }) {
  const starters = [
    { title: '分析项目', prompt: '分析这个项目的结构、核心流程和可以改进的地方' },
    { title: '操作设备', prompt: '查看我的设备状态，并告诉我现在最需要处理的问题' },
    { title: '整理信息', prompt: '帮我整理最近的任务、会话和需要继续推进的事项' },
    { title: '调用工具', prompt: '列出当前最适合使用的工具，并说明各自用途' },
  ];
  return <div className="empty-state product-empty-state">
    <div className="empty-state-center">
      <div className="empty-state-logo" aria-hidden="true">C</div>
      <h1>有什么可以帮忙的？</h1>
      <p>在一个会话里使用模型、工具、附件和任务。</p>
      <div className="starter-grid">
        {starters.map(item => <button key={item.title} type="button" className="starter-card" disabled={busy} onClick={() => setInput(item.prompt)}><b>{item.title}</b><span>{item.prompt}</span></button>)}
      </div>
      <div className="empty-state-secondary-actions">
        {!modelReady ? <button type="button" onClick={() => openSettings('model')}>配置模型</button> : null}
        {hasWorkspaces ? <button type="button" className="secondary" disabled={busy} onClick={openWorkspacePicker}>切换工作空间</button> : null}
      </div>
    </div>
  </div>;
}
