export function buildQuickActions(opts) {
  const { branchCurrent, busy, cloneCurrent, copyCurrentMarkdown, copyText, createSession, current, currentPinned, deleteCurrent, exportCurrent, inputRef, messagesLength, openSettings, openManagementPage, pinCurrent, productDiagnostics, renameCurrent, sendMsg, setProjectFilter, setThemeState, showContextPreview, theme } = opts;
  return [
    ...
    { id: 'settings-projects', title: '项目管理', hint: '管理项目名称、提示词和会话归属', run: () => openManagementPage('projects') },
    ...
    { id: 'settings-automation', title: '定时任务', hint: '创建、运行和暂停自动执行任务', run: () => openManagementPage('scheduled-tasks') },
