export function buildQuickActions(opts) {
  const { branchCurrent, busy, cloneCurrent, copyCurrentMarkdown, copyText, createSession, current, currentPinned, deleteCurrent, exportCurrent, inputRef, messagesLength, openSettings, pinCurrent, renameCurrent, sendMsg, showContextPreview, showSystemPrompt, setThemeState, theme } = opts;
  return [
    ...
    { id: 'settings', title: '打开配置中心', hint: '模型、工具、项目、定时任务和系统设置', run: () => openSettings() },
    ...
    { id: 'settings-projects', title: '项目管理', hint: '维护项目名称、提示词和会话归属', run: () => openSettings('projects') },
    { id: 'settings-automation', title: '定时任务', hint: '创建、运行和暂停自动执行任务', run: () => openSettings('automation') },
