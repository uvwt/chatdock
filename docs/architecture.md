...
web/src/styles/settings.css         配置中心、工作空间、模型、供应商、MCP 表单和自动化表单
...
web/src/styles/legacy-overrides.css 历史视觉兜底层；只迁移旧规则，不再新增业务样式
```

新增样式必须进入对应模块。`legacy-overrides.css` 是过渡层，后续改同一块 UI 时应把相关规则消化回模块文件，而不是继续追加 `!important`。
